...
export class CreateVentasComponent implements OnInit {
  ...
  public load_direcciones_tienda = false;
  ...
  init_direcciones(id:any){
    ...
    this.load_direcciones_tienda = true;
    this._adminService.obtener_direccion_tienda_cliente(this.token).subscribe( response => {
      this.direcciones_tienda = response.data
      this.load_direcciones_tienda = false;
    })
    this._adminService.obtener_direccion_todos_cliente(id,this.token).subscribe( response=>{
