...
const { MERCADOPAGO_APIKEY, GMAIL, PASSWORD, URL_TIENDA } = require('../config');
...
const confirmar_correo = async function(cliente){
    const token = jwt.createToken(cliente)
    fs.readFile(process.cwd() + '/mails/email_confirmacion.html', 'utf8' , (err, html) => {
                                
        if (err) {
            console.error(err);
        }
        
        html = html.replace('{{api_confirmacion}}', `${URL_TIENDA}/verificar-email/${token}`);
...
const obtener_datos_cliente = async function(req,res) {
    try {
        const token = req.headers.authorization;
        
        if(!token) {
            return res.status(403).send({message: 'No se proporcionó el token de autorización'});
        }
        
        const datos = jwt.mostrarDatosToken(token);
        
        res.status(200).send({data:datos});
    } catch (err) {
        console.log(err)
        res.status(200).send({message:'Token no válido',data:undefined});
    }
}


const enviar_email = async function(req,res){
    let data = req.body;
    var clientes_arr = [];
    ...
    if(clientes_arr.length == 0){
        if(data.password){
            const token = await confirmar_correo(data)
            res.status(200).send({message:'¡Verifica tu correo nuevo!',data: token});
        }else{
...
const registro_cliente_tienda = async function(req,res){
    ...
    try {
        ...
        let user = jwt.mostrarDatosToken(token);
        clientes_arr = await Cliente.find({email:user.email});

        if(clientes_arr.length !== 0){
            res.status(200).send({message:'El correo ya existe, intente con otro.', data:user});
            return
        }
        if(user.password){
            bcrypt.hash(user.password,null,null, async function(err,hash){
                if(hash){
                    user.password = hash;
                    var reg = await Cliente.create(user);
...
const listar_productos_nuevos_publico = async function(req,res){
    let reg = await Producto.find({estado: 'Publicado'}).sort({createdAt:-1}).limit(6);
    res.status(200).send({data: reg});
}
...
const obtener_direccion_tienda_cliente  = async function(req,res){
    if(req.user){
        let direcciones = await Direccion.find({cliente:'654f258a68c5425d20d5ba50',status:true});
...
const obtener_orden_cliente = async function(req, res) {
    if(req.user){
        var transaccion_id = req.params['transaccion_id'];
        let reg = await Venta.findOne({transaccion: transaccion_id});
        if (reg) {
            res.status(200).send({data: reg});
        } else {
            res.status(404).send({message: 'No se encontró la transacción'});
        }
    } else {
        res.status(500).send({message: 'NoAccess'});
    }
    ...
    try {
        ...
        readHTMLFile(process.cwd() + '/mails/email_pedido.html', (err, html)=>{
            ...
            var mailOptions = {
                ...
                subject: 'Gracias por tu orden, Chasutech.',
                html: htmlToSend
            };
...
const consultarIDPago = async function(req,res){
    if(req.user){
        var id = req.params['id'];
        var venta = await Venta.find({transaccion:id});
...
const webhook = async (req, res) => {
    ...
    if(req.body.action === 'payment.created'){
        ...
        mercadopago.payment.get(payment_id).then(async data => {

            if(data.body.status === 'approved'){
                const cliente_id = req.query.cliente_id;
                const direccion_id = req.query.direccion_id;
                const cupon_id = req.query.cupon_id;
                const precio_envio = parseInt(req.query.precio_envio);
                const subtotal = parseInt(req.query.subtotal);

                let codigo_cupon = undefined;
                let tipo_descuento = undefined;
                let valor_descuento = 0;
                let total_pagar = subtotal + precio_envio - valor_descuento;
                
                if(cupon_id !== 'undefined'){
                    const cupon = await Cupon.findByIdAndUpdate({_id: cupon_id}, {
                        $inc: {limite: -1}
                    });
                    codigo_cupon = cupon.codigo;
                    tipo_descuento = cupon.tipo;
                    valor_descuento = cupon.valor;
                    total_pagar = subtotal + precio_envio - valor_descuento;
                }

                const carrito_cliente = await Carrito.find({cliente:cliente_id}).populate('producto').populate('variedad');
                let dventa = [];
                let data = {};

                carrito_cliente.forEach(element => {
                    dventa.push({
                ...
                data.cupon = codigo_cupon;
                data.transaccion = payment_id;
                data.currency = 'PEN';
                data.subtotal = subtotal;
                data.total_pagar = total_pagar;
                data.envio_precio = precio_envio;
                data.detalles = dventa;
                data.estado = 'Procesando'
                data.metodo_pago = 'Tarjeta de crédito';

                let venta = await Venta.create(data);

                for(var element of data.detalles){
...
module.exports = {
    ...
    obtener_direccion_tienda_cliente,
