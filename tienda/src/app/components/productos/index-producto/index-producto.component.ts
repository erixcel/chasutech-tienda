...
export class IndexProductoComponent implements OnInit {
              ...
              if(this.filtro_search){
                ...
                this._guestService.listar_productos_publico().subscribe(
                  response=>{
                    ...
                    var term = new RegExp(this.filtro_search.toString().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, ""), 'i');
                    this.productos = this.productos_const.filter(item => term.test(item.titulo.normalize("NFD").replace(/[\u0300-\u036f]/g, "")));
                    this.load_data = false;
  ...
  buscar_por_estrella(){
    if(this.filter_estrella == 'todos'){
      this.productos = this.productos_const;
    }else{
      this.productos = [];
      for(var item of this.productos_const){
        if(item.estrellas == this.filter_estrella){
